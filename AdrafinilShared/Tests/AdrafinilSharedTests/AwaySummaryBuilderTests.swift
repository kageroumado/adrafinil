...
struct AwaySummaryBuilderTests {
    ...
    private var opened: Date { closed.addingTimeInterval(600) }
    ...
    @Test("nothing held at close → no summary")
    func emptyHeldReturnsNil() {
        let s = builder.build(heldAtClose: [], activeTools: [], closedAt: closed, openedAt: opened,
                              peakTemperatureCelsius: nil, thermalCutout: false)
        #expect(s == nil)
    }

    @Test("agents still holding at open land in stillActive; the rest in finished")
    func partitionsByStillActiveTools() throws {
    ...
    @Test("duration spans from acquire time through lid-open")
    func durationFromAcquireToOpen() throws {
        let summary = try #require(builder.build(
    ...
    @Test("peak temperature and thermal-cutout flag pass through")
    func passesThroughPeakAndCutout() throws {
        let summary = try #require(builder.build(
