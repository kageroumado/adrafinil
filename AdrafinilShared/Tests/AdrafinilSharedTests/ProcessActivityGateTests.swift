...
struct ProcessActivityGateTests {
    ...
    @Test func firstSightingIsNeverActive() {
    ...
    @Test func sustainedHighRateIsActive() {
        let gate = ProcessActivityGate()
    ...
    @Test func rateIsMeasuredAgainstPreviousSampleNotFirst() {
        let gate = ProcessActivityGate()
    ...
    @Test func nonPositiveDeltaIsInactive() {
        let gate = ProcessActivityGate()
        _ = gate.isActive(pid: 42, treeCPU: 100, now: t0)
        #expect(gate.isActive(pid: 42, treeCPU: 999, now: t0) == false)   // same instant → dt == 0
    }

    @Test func forgetResetsBaselineForRecycledPid() {
        let gate = ProcessActivityGate()
        _ = gate.isActive(pid: 42, treeCPU: 100, now: t0)
